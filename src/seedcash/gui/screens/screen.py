...
class ButtonListScreen(BaseTopNavScreen):
    ...
    def __post_init__(self):
        ...
        if self.is_bottom_list:
            ...
            button_list_y = self.top_nav.height + int(
                (self.canvas_height - self.top_nav.height - button_list_height) / 2
            )
        ...
        for i, button_option in enumerate(self.button_data):
            ...
            button_kwargs = dict(
                text=_(
                    button_option.button_label
                ),  # Wrap here for just-in-time translations
                active_text=_(
                    button_option.active_button_label
                ),  # Wrap here for just-in-time translations
                icon_name=button_option.icon_name,
                icon_color=(
                    button_option.icon_color
                    if button_option.icon_color
                    else GUIConstants.BUTTON_FONT_COLOR
                ),
                is_icon_inline=True,
                ...
                scroll_y=(
                    self.scroll_y_initial_offset
                    if self.scroll_y_initial_offset is not None
                    else 0
                ),
                ...
                font_name=(
                    button_option.font_name
                    if button_option.font_name
                    else self.button_font_name
                ),
                font_size=(
                    button_option.font_size
                    if button_option.font_size
                    else self.button_font_size
                ),
                font_color=(
                    button_option.button_label_color
                    if button_option.button_label_color
                    else GUIConstants.BUTTON_FONT_COLOR
                ),
    ...
    def _render_visible_buttons(self):
        ...
        for i, button in enumerate(self.buttons):
            ...
            if (
                button_position_y >= self.top_nav.height
...
class LargeButtonScreen(BaseScreen):
    ...
    def __post_init__(self):
        ...
        self.settings_button = IconButton(
            icon_name=SeedSignerIconConstants.SETTINGS,
...
class QRDisplayScreen(BaseScreen):
    ...
    class QRDisplayThread(BaseThread):
        ...
        def run(self):
            ...
            cur_brightness_setting = settings.get_value(
                SettingsConstants.SETTING__QR_BRIGHTNESS_TIPS
            )
            is_brightness_tip_enabled = (
                cur_brightness_setting == SettingsConstants.OPTION__ENABLED
            )
            ...
            while self.keep_running:
                ...
                if (
                    is_brightness_tip_enabled
                    and time.time_ns() - self.tips_start_time.cur_count < duration
                ):
    ...
    def _run(self):
        ...
        Settings.get_instance().set_value(
            SettingsConstants.SETTING__QR_BRIGHTNESS, self.qr_brightness.cur_count
        )
...
class LargeIconStatusScreen(ButtonListScreen):
    ...
    def __post_init__(self):
        ...
        self.status_icon.screen_y = self.top_nav.height - int(
            GUIConstants.COMPONENT_PADDING / 2
        )
...
class WarningScreen(WarningEdgesMixin, LargeIconStatusScreen):
    ...
    status_color: str = GUIConstants.WARNING_COLOR
    status_headline: str = _mft(
        "Privacy Leak!"
    )  # The colored text under the alert icon
