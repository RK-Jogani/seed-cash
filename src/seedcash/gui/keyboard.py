...
class Keyboard:
    ...
    class Key:
        ...
        def render_key(self):
            ...
            if self.is_additional_key:
                if (
                    Keyboard.ADDITIONAL_KEYS[self.code]["font"]
                    == Keyboard.ICON_KEY_FONT
                ):
            ...
            if not self.is_active:
                ...
                rect_color = (
                    self.keyboard.highlight_color
                )  # Render solid background with the UI's hero color
                font_color = "black"
            else:
                if self.is_additional_key:
                    rect_color = "#000"
            ...
            self.keyboard.draw.text(
                (
                    ...
                    self.screen_y
                    + self.keyboard.key_height
                    - int((self.keyboard.key_height - text_height) / 2),
                ),
                self.letter,
                fill=font_color,
                font=font,
    ...
    def __init__(
        ...
        for additional_key in additional_keys:
            additional_key_spaces += additional_key[
                "size"
            ]  # e.g. backspace takes up 2 slots
        if rows * cols < len(charset) + additional_key_spaces:
            raise Exception(
                f"charset will not fit in a {rows}x{cols} layout | additional_keys: {additional_keys}"
            )
        ...
        if input == HardwareButtonsConstants.KEY_RIGHT:
            ...
            if self.selected_key["x"] < 0:
                if Keyboard.WRAP_LEFT in self.auto_wrap:
                    ...
                    self.selected_key["x"] = self.keys[self.selected_key["y"]][
                        -1
                    ].index_x
                else:
                    # Undo selection change and notify controlling loop that we've left
            ...
            new_index_x, new_index_y, keyboard_exit = self.get_key_below(
                self.selected_key["x"], self.selected_key["y"]
            )
            ...
            new_index_x, new_index_y, keyboard_exit = self.get_key_above(
                self.selected_key["x"], self.selected_key["y"]
            )
            self.selected_key["x"] = new_index_x
            self.selected_key["y"] = new_index_y
        ...
        for i, row_keys in enumerate(self.keys):
            for j, key in enumerate(row_keys):
                if selected_letter and key.code == selected_letter:
                    ...
                    return
        raise Exception(
            f"""`selected_letter` "{selected_letter}" not found in keyboard"""
        )
    ...
    def set_selected_key_indices(self, x, y):
        ...
        self.get_key_at(self.selected_key["x"], self.selected_key["y"]).is_selected = (
            True
        )
...
class TextEntryDisplay(TextEntryDisplayConstants):
    ...
    def render(self, cur_text=None, cursor_position=None):
        ...
        draw.rounded_rectangle(
            (0, 0, self.width, self.height), fill=self.background_color, radius=4
        )
        ...
        text_height = (
            -1 * top
        )  # "top" is negative when measuring from baseline; ignoring below baseline
        ...
        if self.cursor_mode == TextEntryDisplay.CURSOR_MODE__BLOCK:
            ...
            draw.text(
                (self.text_offset, self.height - int(text_height / 2)),
                self.cur_text[:-1],
            ...
            draw.rectangle(
                (
                    cursor_block_offset,
                    1,
                    cursor_block_offset + cursor_block_width,
                    self.height - 1,
                ),
                fill=cursor_color,
            )
            draw.text(
                (cursor_block_offset + 1, self.height - int(text_height / 2)),
                self.cur_text[-1],
                fill=GUIConstants.ACCENT_COLOR,
                font=self.font,
                anchor="ls",
            ...
            (left, top, right, bottom) = self.font.getbbox(
                cur_text if cur_text else "", anchor="ls"
            )  # measure from baseline
            text_width = right
            ...
            if end_pos_x < self.width:
                ...
                if (
                    self.text_offset + tw_left + cursor_bar_serif_half_width + 3
                    >= self.width
                ):
                    # Cursor is at the extreme right; have to push the full tw_right off
                    #   the right edge of the display.
                    self.text_offset = self.width - (
                        tw_left + cursor_bar_serif_half_width + 3
                    )
            ...
            draw.text(
                (self.text_offset, self.height - int((self.height - text_height) / 2)),
                self.cur_text,
            ...
            draw.line(
                (cursor_bar_x, 3, cursor_bar_x, self.height - 3), fill=cursor_bar_color
            )
            draw.line(
                (
                    cursor_bar_x - cursor_bar_serif_half_width,
                    3,
                    cursor_bar_x + cursor_bar_serif_half_width,
                    3,
                ),
                fill=cursor_bar_color,
            )
            draw.line(
                (
                    cursor_bar_x - cursor_bar_serif_half_width,
